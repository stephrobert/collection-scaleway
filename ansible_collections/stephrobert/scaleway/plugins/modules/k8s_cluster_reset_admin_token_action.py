...
# Opérations : ResetClusterAdminToken
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: k8s_cluster_reset_admin_token_action
short_description: Perform an action on a Scaleway Kubernetes cluster reset admin token
version_added: 0.8.0
description:
- Reset the admin token for a specific Kubernetes cluster. This will revoke the old admin
  token (which will not be usable afterwards) and create a new one. Note that you will need
  to download the kubeconfig again to keep interacting with the cluster.
author:
