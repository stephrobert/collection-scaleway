...
|---|---|
| `local.scaleway.instance_server_info` | lit une Instance par son identifiant, ou liste une zone entière, toutes pages parcourues |
| `local.scaleway.instance_server_action` | allume, arrête, redémarre ou fige une Instance, et attend l'état visé |
...
```bash
ansible-playbook local.scaleway.inventaire_serveurs -e zone=fr-par-1
ansible-playbook local.scaleway.detail_dun_serveur -e zone=fr-par-1
ansible-playbook local.scaleway.arreter_un_serveur -e zone=fr-par-1 -e server_id=<uuid>
```
